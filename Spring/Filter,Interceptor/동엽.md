...
SpringBoot이면 ****DelegatingFilterProxy도 필요없다.****
...
우리가 자주 쓰던 @PathVariable을 붙이면 path에서 자동으로 파라미터에 맵핑을 해주는게 이걸 ArgumentResolver가 해준다.(@RequestParam, @RequestBody 도 마찬가지)
